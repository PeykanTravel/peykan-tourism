...
export function UnifiedCartProvider({ children }: { children: ReactNode }) {
  ...
  const loadCartFromBackend = useCallback(async () => {
    try {
      ...
      const response = await fetch('http://167.235.140.125:8000/api/v1/cart/', {
        headers: {
  ...
  const addItem = useCallback(async (newItem: Omit<CartItem, 'id' | 'total_price'>): Promise<{ success: boolean; error?: string }> => {
    try {
      if (isAuthenticated) {
        ...
        const response = await fetch('http://167.235.140.125:8000/api/v1/cart/add/', {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const updateItem = useCallback(async (id: string, updates: Partial<CartItem>): Promise<{ success: boolean; error?: string }> => {
    try {
      if (isAuthenticated) {
        ...
        const response = await fetch(`http://167.235.140.125:8000/api/v1/cart/items/${id}/`, {
          method: 'PATCH',
          headers: {
            'Authorization': `Bearer ${token}`,
            'Content-Type': 'application/json',
  ...
  const clearCart = useCallback(async (): Promise<{ success: boolean; error?: string }> => {
    try {
      if (isAuthenticated) {
        ...
        const response = await fetch('http://167.235.140.125:8000/api/v1/cart/clear/', {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${token}`,
          },
