    ...
    return (
      <Link href={`/events/${event.slug}`} className="block group">
        <div className="flex w-full max-w-5xl mx-auto h-56 sm:h-72 rounded-xl shadow-xl overflow-hidden relative" style={{ background: 'linear-gradient(120deg, #dfeaef 0%, #c0cfe4 25%, #bcdee1 50%, #d4e3ca 75%, #e6ced1 100%)' }}>
          ...
          {/* Center: Event info */}
          <div className="flex flex-col justify-center items-start bg-white h-full px-8 py-6 w-2/5 min-w-[220px] gap-6">
            <div className="text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight uppercase leading-tight text-gray-900 mb-2">{event.title}</div>
            <div className="grid grid-cols-2 gap-2 w-full text-base sm:text-lg font-medium text-gray-700">
              <div className="border border-gray-300 rounded px-3 py-2 text-center">27 May</div>
              <div className="border border-gray-300 rounded px-3 py-2 text-center">Fauget Studio</div>
              <div className="border border-gray-300 rounded px-3 py-2 text-center">16.00 - 18.00</div>
              <div className="border border-gray-300 rounded px-3 py-2 text-center">123 Anywhere St., Any City</div>
            </div>
          </div>
          {/* Perforated divider */}
          ...
          <div className="flex flex-row items-center justify-center bg-white h-full px-2 py-4 w-[220px] min-w-[180px] gap-2">
            {/* Col 1: Values */}
            <div className="flex flex-col items-center justify-center gap-2 text-xl sm:text-2xl font-extrabold text-gray-900">
              <div>2F</div>
              <div>G</div>
              <div>10</div>
              <div>25</div>
            ...
            {/* Col 2: Labels */}
            <div className="flex flex-col items-end justify-center gap-2 text-xs sm:text-sm font-bold text-gray-500 pr-1">
              <div>Level</div>
              <div>Sec</div>
              <div>Row</div>
              <div>Seat</div>
            ...
            {/* Col 4: Ticket No */}
            <div className="flex flex-col items-center justify-center gap-1 text-xs sm:text-sm font-bold text-gray-700 pl-1">
              <div>Ticket No:</div>
      ...
      <div className="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-200 border border-gray-200 overflow-hidden">
        ...
        <div className="relative h-48">
          ...
          <div className="absolute top-2 right-2 flex space-x-1">
            ...
            <button
              onClick={handleShare}
              className="p-1.5 rounded-full bg-white/80 text-gray-600 hover:bg-white hover:text-blue-500 transition-colors"
            >
        ...
        {/* Content */}
        <div className="p-4">
          <h3 className="text-lg font-semibold text-gray-900 group-hover:text-blue-600 transition-colors line-clamp-1 mb-2">
            {event.title}
          </h3>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-1">
              {event.artists.slice(0, 2).map((artist, index) => (
                <span key={artist.id} className="text-xs bg-gray-100 text-gray-700 px-2 py-1 rounded">
                  {artist.name}
                </span>
              ))}
